...
class SGColumnNode:
    ...
    @property
    def is_column(self) -> TypeGuard["SGColumnNode"]:
        return True
...
SGEdgeFlowTarget = NamedTuple(
    "SGEdgeFlowTarget", [("dg_target_id", str), ("sg_target_id", str), ("edge_id", str)]
)
# the source of the flow is not needed but still there for what reason? consistent?
SGEdgeFlowSource = NamedTuple(
    "SGEdgeFlowSource", [("dg_source_id", str), ("sg_source_id", str), ("edge_id", str)]
)
...
class SGStatementNode:
    ...
    flow: Dict[
        Tuple[SGEdgeFlowSource, SGEdgeFlowTarget], Dict[str, List[FlowProvenance]]
    ]
    ...
    def track_provenance(
        self,
        source_flow: SGEdgeFlowSource,
        sid: str,
        target_flow: SGEdgeFlowTarget,
    ...
    def compute_freq(
            ...
            if edge.target_id == self.id:
                ...
                for (source_flow, target_flow), stmt2prov in self.flow.items():
                    if (
                        ...
                        or source_flow.sg_source_id != edge.source_id
                    ):
                        continue
                ...
                for (source_flow, target_flow), stmt2prov in self.flow.items():
                    if (
                        target_flow.sg_target_id == edge.target_id
        ...
        if edge.target_id == self.id:
            for (source_flow, target_flow), stmt2prov in self.flow.items():
                if (
                    source_flow.sg_source_id != edge.source_id
...
class SemanticGraphConstructor:
                ...
                for stmt in stmts:
                    ...
                    for tgi, vals in children_values.items():
                        ...
                        if (sgi, stmt_id, p) not in sg.edges:
                            sg.add_edge(
                                sgi, stmt_id, key=p, data=SGEdge(sgi, stmt_id, p, {})
                            )
                        if (stmt_id, tgi, p) not in sg.edges:
                            sg.add_edge(
                                stmt_id, tgi, key=p, data=SGEdge(stmt_id, tgi, p, {})
                            )
    ...
    def construct_sm(self, args: SemanticGraphConstructorArgs):
        ...
        sm = O.SemanticModel()
        readable_label_fn = lambda qnode_id: get_label(
            qnode_id, self.qnodes, self.wdclasses, self.wdprops
        )
        ...
        for uid, udata in args.sg.nodes(data=True):
            ...
            if u.is_column:
                ...
                if u.column in args.cta:
                    ...
                    sm.add_node(
                        O.ClassNode(
                            ...
                            qnode.get_uri(),
                            f"wd:{qnode.id}",
                            readable_label=readable_label_fn(qnode.id),
                        )
                    )
                ...
                sm.add_node(
                    O.ClassNode(u.id, self.STATEMENT_URI, self.STATEMENT_REL_URI)
                )
            elif u.is_value:
        ...
        for uid, vid, eid, edata in args.sg.edges(data=True, keys=True):
            ...
            sm.add_edge(
                O.Edge(
                    ...
                    porq.get_uri(),
                    f"p:{porq.id}",
                    readable_label=readable_label_fn(porq.id),
                )
            )
        args.sm = sm
        ...
        while not stop:
            ...
            for nid, ndata in list(sm_g.nodes(data=True)):
                ...
                if n.is_value and n.is_entity_value:
                    inedges: List[NxSGEdge] = list(
                        sm_g.in_edges(nid, data=True, keys=True)
                    )
                    outedges: List[NxSGEdge] = list(
                        sm_g.out_edges(nid, data=True, keys=True)
                    )

                    if len(inedges) == 0:
                        if all(
                            sm_g.out_degree(vid) == 1
                            for _, vid, teid, tedata in outedges
                        ):
                    ...
                    if len(inedges) > 0:
                        uid, _, seid, sedata = inedges[0]
                        if not all(
                            sm_g.nodes[sib_nid]["data"].is_value
                            for _, sib_nid in sm_g.out_edges(uid)
                        ):
                            continue
            ...
            for nid, ndata in list(sm_g.nodes(data=True)):
                n = ndata["data"]
                if n.is_statement and (
                    sm_g.in_degree(nid) == 0 or sm_g.out_degree(nid) == 0
                ):
                    sm_g.remove_node(nid)
                    stop = False
    ...
    def calculate_link_frequency(self, args: SemanticGraphConstructorArgs):
        def get_in_edges(g, uid) -> List[SGEdge]:
            return [
                edata["data"]
                for _, _, eid, edata in g.in_edges(uid, data=True, keys=True)
            ]
        ...
        for sid, sdata in sg.nodes(data=True):
            ...
            in_edge.features["freq"] = s.compute_freq(
                sg, args.dg, in_edge, is_unique_freq=False
            )
            in_edge.features["unique_freq"] = s.compute_freq(
                sg, args.dg, in_edge, is_unique_freq=True
            )
            ...
            for _, vid, sv_eid, sv_edata in sg.out_edges(sid, data=True, keys=True):
                ...
                sv_e.features["freq"] = s.compute_freq(
                    sg, args.dg, sv_e, is_unique_freq=False
                )
                sv_e.features["unique_freq"] = s.compute_freq(
                    sg, args.dg, sv_e, is_unique_freq=True
                )
    ...
    def st_maximum_arborescence_solver(self, args: SemanticGraphConstructorArgs):
        ...
        # find the tree
        resp = nx.algorithms.tree.branchings.maximum_branching(
            args.sg, attr="weight", preserve_attrs=True
        )
                ...
                if n.is_statement and (outdegree == 0 or indegree == 0):
                    ...
                    if outdegree == 0:
                        ...
                        total_outdegree_parent = sum(
                            sm_g.out_degree(uid)
                            for uid, vid, edata in sm_g.in_edges(nid, data=True)
                        )
                        ...
                        if all(
                            sm_g.in_degree(uid) == 1
                            and eid
                            != next(iter(sm_g.in_edges(uid, data=True, keys=True)))[2]
                            for uid, vid, eid, edata in sm_g.in_edges(
                    ...
                    if indegree == 0:
                        ...
                        total_outdegree_children = sum(
                            sm_g.out_degree(uid)
                            for uid, vid, eid in sm_g.out_edges(nid, data=True)
                        )
    ...
    def st_networkx_st_solver(self, args: SemanticGraphConstructorArgs):
        ...
        return nx.algorithms.approximation.steinertree.steiner_tree(
            args.sg, terminal_nodes, weight="weight"
        )
    ...
    def edge_fn(eid, edata):
        ...
        if len(edge.features) > 0:
            ...
            for feat, val in edge.features.items():
                html_feat.append(
                    f"<li><span style='min-width: 100px'><b>{feat}:</b></span> {val:.2f}</li>"
                )
...
if __name__ == "__main__":
    ...
    if kg_index_file.exists():
        ...
        kg_object_index = KGObjectIndex.from_qnodes(
            index_qnode_ids, qnodes, wdprops, n_hop=2, verbose=True
        )
